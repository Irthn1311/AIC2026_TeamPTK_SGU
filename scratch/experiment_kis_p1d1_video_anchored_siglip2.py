#!/usr/bin/env python3
"""KIS P1D1: Video-Anchored SigLIP2 Frame Rerank Shootout.

Architecture:
  - Video Anchoring (Coarse Anchor): Marian P0 -> OpenAI CLIP 1st-pass determines the immutable VIDEO ranking.
  - Within-Video Frame Discrimination (Fine Reranker): For each video in CLIP's top list, SigLIP2 re-orders
    the candidate frames strictly WITHIN that video.
  - Strict Guarantee: SigLIP2 cannot swap or demote videos (e.g. cannot replace Tiger video with Lion Dance).
    SigLIP2 only selects the best frame/viewpoint within the video already anchored by CLIP.

Compares:
  - ARM A (Production P0 Baseline): Marian EN -> OpenAI CLIP 1st-pass coarse frame order.
  - ARM B (Video-Anchored SigLIP2): Exact same candidate pool, preserving CLIP's video hierarchy, with within-video frames re-ordered purely by SigLIP2.

Strict constraints:
  - Production P0 remains 100% immutable (default OFF).
  - ZERO image re-indexing: 100% reuse of existing 177,532-keyframe OpenAI CLIP ViT-B/32 index for coarse search.
  - Zero uncalibrated score fusion.
  - Hard 30/30 frame decode coverage gate.
  - Marian Parity Gate: 3/3 exact string parity after strip on p1-2, p1-10, p1-12.
  - Strict SigLIP2 model requirement: google/siglip2-base-patch16-224 (no fallback).
...
def load_strict_siglip2_model(device: str) -> tuple[Any, Any]:
    ...
    try:
        ...
        print("=" * 120, flush=True)
        raise RuntimeError(f"P1D1 Aborted: Failed to load {SIGLIP2_MODEL_ID}: {exc}") from exc
...
def perform_video_anchored_rerank(
    candidates_a: list[Any],
    b64_thumbs: list[str],
    siglip_scores: np.ndarray,
) -> tuple[list[Any], list[str], list[float], list[str]]:
    """Re-order candidate frames strictly WITHIN each video block while preserving CLIP's video hierarchy."""
    # 1. Group candidate indices by video_id in the exact order of their first appearance in CLIP Top 30
    video_order: OrderedDict[str, list[int]] = OrderedDict()
    for idx, c in enumerate(candidates_a):
        video_order.setdefault(c.video_id, []).append(idx)

    # 2. Within each video block, sort its frame indices by descending SigLIP2 score
    final_indices: list[int] = []
    video_shifts_summary: list[str] = []

    for vid, indices in video_order.items():
        sorted_indices = sorted(indices, key=lambda i: siglip_scores[i], reverse=True)
        final_indices.extend(sorted_indices)
        
        # Track if SigLIP2 selected a different top frame for this video
        orig_top_idx = indices[0]
        new_top_idx = sorted_indices[0]
        if orig_top_idx != new_top_idx:
            orig_f = candidates_a[orig_top_idx].frame_id
            new_f = candidates_a[new_top_idx].frame_id
            video_shifts_summary.append(f"{vid}: f={orig_f}(orig) -> f={new_f}(siglip, s={siglip_scores[new_top_idx]:.3f})")

    candidates_b = [candidates_a[i] for i in final_indices]
    b64_thumbs_b = [b64_thumbs[i] for i in final_indices]
    siglip_scores_b = [float(siglip_scores[i]) for i in final_indices]

    # Verify video ordering is 100% preserved
    unique_vids_a = list(OrderedDict.fromkeys(c.video_id for c in candidates_a))
    unique_vids_b = list(OrderedDict.fromkeys(c.video_id for c in candidates_b))
    assert unique_vids_a == unique_vids_b, "FATAL ERROR: Video hierarchy was altered in video-anchored reranking!"

    # Verify candidate sets are 100% equal
    assert set((c.video_id, c.frame_id) for c in candidates_a) == set((c.video_id, c.frame_id) for c in candidates_b)

    return candidates_b, b64_thumbs_b, siglip_scores_b, video_shifts_summary


def run_p1d1_experiment() -> None:
    print("=" * 150, flush=True)
    print("KIS P1D1: VIDEO-ANCHORED SIGLIP2 FRAME RERANK SHOOTOUT", flush=True)
    print("=" * 150, flush=True)
    print("CORE PRINCIPLE:", flush=True)
    print("  • OpenAI CLIP locks the immutable VIDEO ranking (Entity Anchor).", flush=True)
    print("  • SigLIP2 only re-orders frames WITHIN each video (Viewpoint / Composition Refinement).", flush=True)
    print("  • Zero score fusion, Zero video swapping, Zero candidate injection.", flush=True)
    print("=" * 150, flush=True)

    yaml_path = REPO_ROOT / "systems" / "system_tai" / "configs" / "production.yaml"
    input_root = Path("/kaggle/input/datasets") if Path("/kaggle/input/datasets").exists() else Path("/kaggle/input")
    reuse_manifest = get_reuse_manifest()
    out_dir = Path("/kaggle/working/output/kis_p1d1_session") if Path("/kaggle/working").exists() else REPO_ROOT / "scratch" / "kis_p1d1_session"
    ...
    print("\n" + "=" * 150, flush=True)
    print(f"RUNNING VIDEO-ANCHORED SIGLIP2 FRAME RERANK ACROSS 18 BTC KIS QUERIES", flush=True)
    print("=" * 150, flush=True)
    ...
    for idx, item in enumerate(BTC_KIS_QUERIES, start=1):
        ...
        # Step D: Video-Anchored Re-ordering
        candidates_b, b64_thumbs_b, siglip_scores_b, within_video_shifts = perform_video_anchored_rerank(
            candidates_a,
            b64_thumbs,
            siglip_scores,
        )
        ...
        print(f"• Video Anchors (Top 3): {[c.video_id for c in candidates_a[:3]]}", flush=True)
        print(f"• Arm A (CLIP Top 3)     : {top10_desc_a[:3]}", flush=True)
        print(f"• Arm B (SigLIP2 Anchored): {top10_desc_b[:3]}", flush=True)
        if within_video_shifts:
            print(f"• Within-Video Refinements: {within_video_shifts[:3]}", flush=True)
        ...
        if qid == "query-p1-24-kis":
            ...
            print(f"  🔍 PROTECTED GUARD (p1-2 Tiger Anchor): Video #1 is {candidates_b[0].video_id} (Lion Dance prevented ✅)", flush=True)

    # Generate comparative HTML gallery
    gallery_out = Path("/kaggle/working/kis_p1d1_video_anchored_gallery.html")
    generate_anchored_gallery_html(results, gallery_out, SIGLIP2_MODEL_ID)
    print(f"\nSaved Comparative Side-by-Side Gallery to: {gallery_out}", flush=True)

    # Summary table
    print("\n" + "=" * 150, flush=True)
    print("KIS P1D1 VIDEO-ANCHORED SIGLIP2 FRAME RERANKING OVERVIEW TABLE", flush=True)
    print("=" * 150, flush=True)
    print(f"{'Query ID':<18} | {'Category':<22} | {'Arm A CLIP Top 1':<24} | {'Arm B Anchored Top 1':<24} | {'Video Anchored':<15} | {'Total Lat':<10}")
    print("-" * 125)
    for r in results:
        ...
        same_vid = "EXACT MATCH ✅" if r['candidates_a'][0].video_id == r['candidates_b'][0].video_id else "MISMATCH ❌"
        print(f"{r['qid']:<18} | {r['category']:<22} | {top1_a:<24} | {top1_b:<24} | {same_vid:<15} | {r['lat_total']*1000:6.0f} ms")
    print("=" * 150, flush=True)
...
def generate_anchored_gallery_html(results: list[dict[str, Any]], out_path: Path, model_id: str) -> None:
    html_cards = []
    for r in results:
        qid = r["qid"]
        name = r["name"]
        ...
        grid_a = render_top3_grid(preds_a, thumbs_a, "ARM A: CLIP Coarse Video & Frame Order (Top 3)", "#0d6efd", is_siglip=False)
        grid_b = render_top3_grid(preds_b, thumbs_b, "ARM B: Video-Anchored SigLIP2 Frame Order (Top 3)", "#28a745", is_siglip=True)
        ...
        shifts_html = f'<div style="font-size:11px; color:#98c379; margin-top:6px;"><b>Within-Video Refinements:</b> {", ".join(shifts)}</div>' if shifts else '<div style="font-size:11px; color:#888; margin-top:6px;"><b>Within-Video Refinements:</b> Same top frames as CLIP</div>'
    ...
    <!DOCTYPE html>
    <html>
    <head><meta charset="utf-8"><title>KIS P1D1 Video-Anchored SigLIP2 Gallery</title></head>
    <body style="background:#121212; color:#fff; font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif; padding:20px;">
        <h2 style="color:#61afef; border-bottom:2px solid #333; padding-bottom:8px;">🔬 KIS P1D1: VIDEO-ANCHORED SIGLIP2 FRAME RERANK SHOOTOUT GALLERY</h2>
        <div style="color:#aaa; font-size:13px; margin-bottom:16px;">
            <b>Architecture:</b> CLIP locks immutable Video Ranking (Entity Anchor). SigLIP2 re-orders candidate frames strictly within each video.
        </div>
        {''.join(html_cards)}
    </body>
    </html>
