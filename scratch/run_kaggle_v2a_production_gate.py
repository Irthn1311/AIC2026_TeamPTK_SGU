...
Evaluates Legacy vs V2-A.1 on REAL Kaggle production CLIP feature stores (873 videos, 177,321 rows):
...
def run_kaggle_production_gate() -> None:
    ...
    # Add query-p1-1-kis (Ground Truth L30_V046 frame 97)
    extra_p1_query = {
    ...
    all_eval_queries = list(dev_gt_queries) + [extra_p1_query]
    sidecar_records["query-p1-1-kis"] = {
        "source_vi": "Cảnh quay một nhóm hơn 5 người đang cùng nhau tập thể dục hai tay chạm mũi chân, chỉ có một người đeo kính và có ba người đội nón màu đỏ.",
        "translation_en": "A scene of a group of more than 5 people exercising together touching their toes with both hands, only one person wearing glasses and three people wearing red hats.",
    }
    ...
    print("=" * 120, flush=True)
    print(f"• Git Commit HEAD               : {get_git_head()}", flush=True)
    ...
    print(f"• Total Queries in Benchmark    : {len(all_eval_queries)} (38 DEV + 1 BTC Ground Truth)", flush=True)

    # =========================================================================
    # 1. RUN PAIRED RETRIEVAL (LEGACY vs V2-A.1)
    # =========================================================================
    results_legacy = []
    results_v2a = []
    latencies_leg = []
    ...
    print("\n" + "=" * 120, flush=True)
    print("EXECUTING PAIRED RUN ON KAGGLE PRODUCTION INDEX...", flush=True)
    print("=" * 120, flush=True)

    for idx, q in enumerate(all_eval_queries, start=1):
        qid = q["query_id"]
        ...
        print(f"[{idx:02d}/{len(all_eval_queries):02d}] {qid:<16} | Tgt: {target_vid:<8} | Leg Rank: {leg_target_rank:<4} | V2-A: {v2_target_rank:<4} ({diff_str:<3}) | FrameHit: Leg={leg_first_hit:<4} V2={v2_first_hit:<4} | K={chosen_k:<2} | H={h_norm:.3f} d1_5={d1_5:.3f} | {', '.join(reasons)}", flush=True)
    ...
    # =========================================================================
    print("\n" + "=" * 120, flush=True)
    print("FINAL REAL KAGGLE PRODUCTION GATE SUMMARY", flush=True)
    print("=" * 120, flush=True)

    n_q = len(results_v2a)
    for k_val in (8, 16, 32, 48, 64):
        hit_leg = sum(1 for r in results_legacy if r["rank"] <= k_val) / n_q * 100
        hit_v2 = sum(1 for r in results_v2a if r["rank"] <= k_val) / n_q * 100
        print(f"• VideoHit@{k_val:<2} : Legacy = {hit_leg:5.1f}% | KIS V2-A.1 = {hit_v2:5.1f}% (Delta: {hit_v2 - hit_leg:+.1f}%)", flush=True)

    print("\n--- PHYSICAL FRAME RECALL (OFFICIAL KIS EVALUATOR) ---", flush=True)
    for k_val in OFFICIAL_K:
        r_leg = sum(1 for r in results_legacy if r["frame_hit"] <= k_val) / n_q * 100
        r_v2 = sum(1 for r in results_v2a if r["frame_hit"] <= k_val) / n_q * 100
    ...
    print("\n--- ADAPTIVE-K DISTRIBUTION ---", flush=True)
    print(f"• K=32 (Confident Default)     : {k32_cnt}/{n_q} ({k32_cnt/n_q*100:.1f}%)", flush=True)
    print(f"• K=48 (Moderate / Attributes) : {k48_cnt}/{n_q} ({k48_cnt/n_q*100:.1f}%)", flush=True)
    print(f"• K=64 (High Uncertainty / Flat): {k64_cnt}/{n_q} ({k64_cnt/n_q*100:.1f}%)", flush=True)

    h_sorted = sorted([r["h"] for r in results_v2a])
    d_sorted = sorted([r["d1_5"] for r in results_v2a])
    p90_idx = int(0.90 * n_q)
    print("\n--- ROBUST ENTROPY & MARGINS (MAD STANDARDIZED) ---", flush=True)
    print(f"• Robust Entropy H_norm : min={h_sorted[0]:.4f}, median={h_sorted[n_q//2]:.4f}, p90={h_sorted[p90_idx]:.4f}, max={h_sorted[-1]:.4f}", flush=True)
    print(f"• Top1-Top5 Margin      : min={d_sorted[0]:.4f}, median={d_sorted[n_q//2]:.4f}, p90={d_sorted[p90_idx]:.4f}, max={d_sorted[-1]:.4f}", flush=True)
    ...
    print(f"\n• Target Rank Improvements : {len(improvements)} / {n_q} ({len(improvements)/n_q*100:.1f}%)", flush=True)
    print(f"• Target Rank Regressions  : {len(regressions)} / {n_q} ({len(regressions)/n_q*100:.1f}%)", flush=True)
