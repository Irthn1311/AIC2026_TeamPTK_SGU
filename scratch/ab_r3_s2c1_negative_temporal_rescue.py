# ==============================================================================================================
# Phase R3-S2C1: Bounded Negative Temporal Tail Rescue Targeted A/B Validation Runner
# ==============================================================================================================
...
def run_ab_validation(
    ...
    print("=" * 135)
    print("ROUND-3 SPRINT 2C.1: BOUNDED NEGATIVE TEMPORAL TAIL RESCUE A/B EXPERIMENT")
    ...
    session_output = Path("/kaggle/working/output/ab_s2c1") if Path("/kaggle/working").exists() else REPO_ROOT / "scratch" / "ab_s2c1"
    ...
    base_evidence_config = QAVideoConditionedEvidenceConfig(
        ...
        bounded_negative_temporal_rescue_enabled=False,  # Default OFF
        bounded_negative_temporal_rescue_offsets=(-100, -200, -300),
        bounded_negative_temporal_rescue_tail_budget=5,
    )
    ...
    for qid in target_queries:
        ...
        # ---------------------------------------------------------------------
        # ARM A: CONTROL (Negative Temporal Rescue OFF)
        # ---------------------------------------------------------------------
        preds_ctrl = []
        ctrl_hit_rank = None
        ctrl_hit_frame = None
        ...
        if not treatment_only:
            ...
            req_ctrl = QAQueryRequest(
                request_id=f"r3s2c1-control-{qid}",
                query_id=qid,
        ...
        # ---------------------------------------------------------------------
        # ARM B: TREATMENT (Negative Temporal Rescue ON, offsets=[-100, -200, -300], tail_budget=5)
        # ---------------------------------------------------------------------
        runtime.qa_pipeline.video_conditioned_evidence_config = replace(
            base_evidence_config,
        ...
        req_treat = QAQueryRequest(
            request_id=f"r3s2c1-treatment-{qid}",
            query_id=qid,
            event_description=q_vi,
            question=q_vi,
            event_description_en=q_en if q_en else None,
        ...
        rescue_meta = diags_treat.get("bounded_negative_temporal_rescue", {})
        admitted_tuples = rescue_meta.get("admitted_tail_tuples", []) if "admitted_tail_tuples" in rescue_meta else [
            t for t in preds_treat if str(t.get("slot_source", "")).startswith("RESCUE_TAIL_BOUNDED_NEGATIVE_TEMPORAL_RESCUE")
        ]
        ...
        print(f"  - Source Video                : {rescue_meta.get('source_video')}")
        print(f"  - Source Refined Anchor Frame : {rescue_meta.get('source_anchor_frame')} ({rescue_meta.get('source_refinement_status')})")
        print(f"  - Evaluated Offsets           : {rescue_meta.get('offsets_evaluated')}")
        print(f"  - Valid Physical Frame IDs    : {rescue_meta.get('valid_offset_frames')}")
        print(f"  - Decoded Frame Count         : {len(rescue_meta.get('decoded_frames', []))} -> {rescue_meta.get('decoded_frames')}")
        print(f"  - Exact Answer Route Used     : {rescue_meta.get('answer_route_used')}")
        ...
        # DEV Ground Truth Evaluation for offset frames
        print(f"\n  --- DEV GROUND TRUTH EVALUATION FOR PROBE FRAMES ---")
        for cand_f in rescue_meta.get("valid_offset_frames", []):
            dist = interval_distance(cand_f, start_f, end_f)
            in_gt = (start_f <= cand_f <= end_f)
            offset_from_src = cand_f - int(rescue_meta.get('source_anchor_frame', 0))
            print(f"  • Frame {cand_f} (Offset {offset_from_src:+d}): Distance to GT [{start_f}..{end_f}] = {dist} frames | In GT? {'YES ✅' if in_gt else 'NO ❌'}")
        ...
        results_table.append({
            ...
            "source_anchor": rescue_meta.get('source_anchor_frame'),
    ...
    # =========================================================================
    print("\n" + "=" * 135)
    print("SPRINT 2C.1 BOUNDED NEGATIVE TEMPORAL RESCUE: A/B SUMMARY TABLE")
    print("=" * 135)
    print(f"{'Query ID':<8} | {'Target':<10} | {'Control Hit':<14} | {'Treatment Hit':<14} | {'Source Anchor':<15} | {'Tail Admits':<12} | {'Prefix 1..95 Parity'}")
    print("-" * 135)
    for r in results_table:
        print(f"{r['qid']:<8} | {r['target']:<10} | {r['ctrl_hit']:<14} | {r['treat_hit']:<14} | {str(r['source_anchor']):<15} | {str(r['admitted_tail_count']) + ' tuples':<12} | {r['prefix_parity']}")
    print("=" * 135)


if __name__ == "__main__":
    default_input = Path("/kaggle/input/datasets") if Path("/kaggle/input/datasets").exists() else Path("/kaggle/input")
    parser = argparse.ArgumentParser(description="Run R3-S2C1 Bounded Negative Temporal Rescue A/B Experiment")
