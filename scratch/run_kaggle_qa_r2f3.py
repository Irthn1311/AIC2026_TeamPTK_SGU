# ==============================================================================================================
# QA-R2F3 PRODUCTION A/B EXPERIMENT: TIER 3 NEGATIVE-OFFSET-FIRST INTERLEAVING (SINGLE-VARIABLE ISOLATION)
# Target Branch: feat/system-tai-quality-q1b
# Exact Commit: 9e1b6c7
# Both Arms: --qa-temporal-seeds-per-video 2 --qa-secondary-temporal-micro-budget --qa-primary-11-12-micro-coverage --qa-tier3-primary-first
# Control:   tier3_negative_offset_first OFF (direct -> +30 -> -30)
# Treatment: tier3_negative_offset_first ON  (direct -> -30 -> +30)
# ==============================================================================================================
...
# ==============================================================================================================
# STEP 1: GIT REPOSITORY SYNC TO EXACT COMMIT 9e1b6c7
# ==============================================================================================================
print("\n" + "=" * 110)
print("STEP 1: SYNCING CODEBASE TO EXACT COMMIT 9e1b6c7")
print("=" * 110)

REPO_DIR = Path("/kaggle/working/AIC2026_TeamPTK_SGU")
REPO_URL = "https://github.com/Irthn1311/AIC2026_TeamPTK_SGU.git"
TARGET_COMMIT = "9e1b6c7"
...
def run_fast_arm(enable_negative_offset_first: bool, out_dir: Path) -> None:
    if out_dir.exists():
    ...
    if enable_negative_offset_first:
        cmd.append("--qa-tier3-negative-offset-first")

    desc = f"t3_primary_first=ON, t3_neg_first={'ON (-30 before +30)' if enable_negative_offset_first else 'OFF (+30 before -30)'}"
...
# ==============================================================================================================
# STEP 3: EXECUTE CLEAN CONTROL (NEG_FIRST=OFF) & TREATMENT (NEG_FIRST=ON)
# ==============================================================================================================
print("\n" + "=" * 110)
print("STEP 3: EXECUTING CLEAN CONTROL (NEG_FIRST=OFF) & TREATMENT (NEG_FIRST=ON)")
print("=" * 110)

CONTROL_DIR = Path("/kaggle/working/output/qa_r2f3_control_pos_first")
TREATMENT_DIR = Path("/kaggle/working/output/qa_r2f3_treatment_neg_first")

print("Executing clean Control (t3_primary=ON, t3_neg_first=OFF)...")
run_fast_arm(enable_negative_offset_first=False, out_dir=CONTROL_DIR)

print("Executing clean Treatment (t3_primary=ON, t3_neg_first=ON)...")
run_fast_arm(enable_negative_offset_first=True, out_dir=TREATMENT_DIR)
...
print("Evaluating Control (t3_primary=ON, t3_neg_first=OFF)...")
ctrl_eval = evaluate_run(CONTROL_DIR)
print("Evaluating Treatment (t3_primary=ON, t3_neg_first=ON)...")
...
inspect_query("QA-13", "QA-13 (Target: expect rank 28 -> <=20, expected @19)")
inspect_query("QA-08", "QA-08 (Expect rank <= 50, expected @43)")
inspect_query("QA-27", "QA-27 (Expect rank <= 50, expected @49)")
inspect_query("QA-46", "QA-46 (Expect rank <= 100, expected @78)")
inspect_query("QA-45", "QA-45 (Expect rank <= 100, expected @96)")
...
# Verify Control matches established QA-R2F2 treatment reference
r2f2_ref_hits = {"QA-08": 43, "QA-13": 28, "QA-27": 49, "QA-46": 78, "QA-45": 96}
ctrl_match_ref = (ctrl_hits == r2f2_ref_hits)
print(f"\nControl Baseline Verification vs R2F2 Reference ({r2f2_ref_hits}):")
...
print("\n" + "-" * 110)
print(f"{'Metric':<30} | {'Control (T3_NEG_FIRST OFF)':<33} | {'Treatment (T3_NEG_FIRST ON)':<33} | {'Delta'}")
print("-" * 110)
...
# ==============================================================================================================
print("\n--- QA-R2F3 FINAL ACCEPTANCE CLASSIFICATION ---")
