...
- `validated-only`: score only queries whose mapping evidence is entirely `VALIDATED`.
...
reorders predicted events. R@k is the maximum score in the ranked prefix, and Final
Score is the mean of the five R@k values.

## Workflow

1. Re-import the untracked source DOCX with `scripts/l21_150_import.py`.
2. Produce mapping evidence with `scripts/l21_150_validate_mapping.py`.
