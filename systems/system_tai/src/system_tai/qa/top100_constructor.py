    ...
    *,
    ...
    if policy == "temporal_dense_v1":
        ...
        if tier3_primary_first:
            ...
            # Phase T3-B and T3-C: ±30 sign phase ordering
            t3_offsets = [-30, 30] if tier3_negative_offset_first else [30, -30]
            for offset_val in t3_offsets:
                if len(predictions) >= target_k:
                    break
                for idx, c in enumerate(scored_candidates[1:10], start=2):
                    if len(predictions) >= target_k:
                    ...
                    if not c.get("answers"):
                        continue
                    max_f = c.get("total_frames") or c.get("max_frame_id")
                    _try_add(c["video_id"], c["frame_id"] + offset_val, c["answers"][0], max_frame=max_f, candidate_rank=idx, slot_source="TIER3_OFFSET", offset_frames=offset_val)
        else:
            for idx, c in enumerate(scored_candidates[1:10], start=2):
                if not c.get("answers"):
                    continue
