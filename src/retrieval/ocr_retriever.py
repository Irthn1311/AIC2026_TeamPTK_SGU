"""
In-Memory OCR Retriever — Fast local text search over extracted OCR JSON files.
...
Input: Directory containing L{XX}_{V}.json files generated by OCRExtractor.
...
class InMemoryOCRRetriever(BaseRetriever):
    ...
    def load(self) -> "InMemoryOCRRetriever":
        """Load all per-video JSON files in ocr_dir into memory."""
        ...
        logger.info(f"[InMemoryOCRRetriever] Loading {len(json_files)} OCR JSON files from {self.ocr_dir}...")
        ...
        for json_path in json_files:
            try:
                ...
                for kf in keyframes:
                    ...
                    # If meta_store is provided, complement missing values
        ...
        logger.info(
            ...
            f"across {len(json_files)} videos."
        )
        return self
